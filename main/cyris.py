...
#                            args = program["args"]
    ...
    def os_system(self, filename, command):
        ...
        if filename:
            ...
            return_value = os.system("{0}".format(command, filename))
        else:
	...
	# Deal with AWS
        ...
        if basevm_type == "kvm":
            ...
            if os.stat(scp_host_file).st_size != 0:
                command = ""
                parallel_scp_command = "parallel-scp -l cyuser -e error -O StrictHostKeyChecking=no -O UserKnownHostsFile=/dev/null -h {0} -p {1}".format(scp_host_file, PSCP_CONCURRENCY)
                for guest in self.guests:
                    # NOTE: The name of the file in which a guest base VM image is stored is given by getGuestId()
                    #       However, the name of the VM itself, as registered in KVM, and the associated XML file
